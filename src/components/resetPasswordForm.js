...
export function PasswordResetForm({ popup, popupMsg, setLoading }) {
  ...
  const VerifyHandler = (v) => {
    ...
    fetch(
      `https://password-reset-task3.herokuapp.com/users/resetpwd/${name}/${token}`,
      {
        method: "GET",
        headers: {
  ...
  return (
    <>
      <div
        id="verifyToken"
        style={{
          backgroundColor: Verified ? "rgb(93, 250, 93)" : "",
          color: Verified ? "rgb(93, 250, 93)" : "",
          display: verifyDis
        }}
      >
        <button
          onClick={VerifyHandler}
          style={{ color: Verified ? "rgb(93, 250, 93)" : "" }}
        >
          {Verified ? (
            <>
              <span>Verified </span>
